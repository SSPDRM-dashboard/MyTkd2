...
if (ptIndex !== -1 && onsiteIndex !== -1) {
    ...
    // 2. Remove "inspected" blocks in ptView
    // blue
    ptView = ptView.replace(/\{b\?\.data\.blue_inspected[^}]*\}[^}]*\}[^}]*\}[^}]*\}[^}]*\}[^}]*\}[^}]*\}/g, '');
    ...
    // 3. Resize middle column and text, resize right column to give more space to active table
    ptView = ptView.replace(
    ...
    console.log('Did not find functions');
}
