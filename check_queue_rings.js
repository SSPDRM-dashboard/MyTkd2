...
async function run() {
  ...
  if (data.fields && data.fields.value && data.fields.value.arrayValue) {
      ...
      queue.forEach(q => {
          ...
          if (boutStr.includes('C17') || boutStr.includes('C18') || boutStr.includes('C19') || boutStr.includes('C20') || boutStr.includes('C21') || boutStr.includes('C22')) {
            console.log(`Bout: ${boutStr}, Ring: ${ring}`);
          }
      });
  } 
}
